...
public class MySwitch extends CompoundButton {
	...
	private int withTextInterval = 0; // 文字和按钮之间的间距
	...
	private void init() {
		...
		withTextInterval = 16 * 2;
		setDrawables(getResources().getDrawable(R.drawable.switch_frame), getResources().getDrawable(R.drawable.selector_switch_state), getResources()
				.getDrawable(R.drawable.switch_state_mask), getResources().getDrawable(R.drawable.selector_switch_slider));
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		case MeasureSpec.AT_MOST:// 如果widthSize是当前视图可使用的最大宽度,android:layout_width="wrap_content"
			measureWidth = getCompoundPaddingLeft() + getCompoundPaddingRight();
			break;
		case MeasureSpec.EXACTLY:// 如果widthSize是当前视图可使用的绝对宽度
			measureWidth = MeasureSpec.getSize(widthMeasureSpec);//android:layout_width="100dp",android:layout_width="match_content"
			break;
	...
	protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		buttonRectF.set(buttonLeft, buttonTop, buttonLeft + (frameDrawable != null ? frameDrawable.getIntrinsicWidth() : 0), buttonTop
				+ (frameDrawable != null ? frameDrawable.getIntrinsicHeight() : 0));
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (stateDrawable != null && stateMaskDrawable != null) {
			...
			if (stateMaskDrawable != null && stateBitmap != null && !stateBitmap.isRecycled()) {
				...
				int src = canvas.saveLayer(0, 0, getWidth(), getHeight(), paint, Canvas.MATRIX_SAVE_FLAG | Canvas.CLIP_SAVE_FLAG
	...
	public void setDrawableResIds(int frameDrawableResId, int stateDrawableResId, int stateMaskDrawableResId, int sliderDrawableResId) {
		if (getResources() != null) {
			setDrawables(getResources().getDrawable(frameDrawableResId), getResources().getDrawable(stateDrawableResId),
					getResources().getDrawable(stateMaskDrawableResId), getResources().getDrawable(sliderDrawableResId));
		}
	}
