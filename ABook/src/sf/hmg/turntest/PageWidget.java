...
public class PageWidget extends View {
	...
	private void calcPoints() {
		...
		// 当mBezierStart1.x < 0或者mBezierStart1.x > screenWidth时
		// 如果继续翻页，会出现BUG故在此限制
	...
	private void startAnimation(int delayMillis) {
		...
		if (mCornerY > 0) {
			dy = (int) (mHeight - (mTouch.y + 0.01f));// mTouch.y不能等于mHeight
		} else {
			dy = (int) (1 - mTouch.y); // mTouch.y不能等于0
		}
		// MyLog.i("startAnimation " + dx + " " + dy);
	...
	public boolean canDragOver() {
		mTouchToCornerDis = (float) Math.hypot((mTouch.x - mCornerX), (mTouch.y - mCornerY));
		// MyLog.i("canDragOver "+mTouch.x+" "+mCornerX+" "+mTouch.y+" "+mCornerY);
